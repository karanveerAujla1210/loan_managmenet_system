          ...
          <nav className="mt-6 flex-1 px-4 space-y-2 bg-white">
            {navigation.map((item) => (
              <NavLink key={item.name} to={item.href} end>
                {({ isActive }) => (
                  <div
                    className={`${isActive ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'} group flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200`}
                    aria-current={isActive ? 'page' : undefined}
                  >
                    <item.icon
                      className={`${isActive ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-500'} mr-4 flex-shrink-0 h-5 w-5`}
                      aria-hidden="true"
                    />
