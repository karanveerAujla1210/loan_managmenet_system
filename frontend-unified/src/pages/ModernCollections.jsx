...
import { Phone, Filter, Loader, Search, Download } from 'lucide-react';
...
export default function ModernCollections() {
  ...
  const fetchAllCases = async () => {
    try {
      ...
      const cases = loans.map((loan) => ({
        ...
        nextDue: 'N/A',
        callStatus: 'pending',
        remainingAmount: loan.principalAmount || 0,
        dpd: 0,
        status: loan.status,
        branch: loan.branch || 'All',
        mobileNo: 'N/A',
        email: loan.customerEmail || 'N/A',
      }));
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      await paymentsService.recordPaymentWithUpdate(
        selectedCase.loanId,
        {
  ...
  const exportToCSV = () => {
    const csv = [
      ['Loan ID', 'Customer', 'Amount', 'Remaining', 'DPD', 'Bucket', 'Status', 'Last Payment', 'Next Due'],
      ...filteredCases.map(c => [
        c.loanId,
        c.customer,
        c.amount,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 mt-1">Track {allCases.length} disbursed cases</p>
        </div>
        <button
          onClick={exportToCSV}
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="text-lg font-medium text-gray-900">{selectedCase.status}</p>
                </div>
              </div>
              ...
              <div className="flex space-x-3 pt-4 border-t">
                <button
                  onClick={() => toast.info('Call feature requires phone integration')}
